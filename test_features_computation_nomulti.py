...
from library_25_11_2013 import Read_Image,spectral_features,textural_features,multispectral_features,watershed_otb,Shp2Rast,Read_Image_Parameters
...
path = 'C:\\workspace\\Sensum\\Izmir\\Landsat5\\1995\\'
input_file = 'LT51800331995162RSA00_rgb.TIF'    #name of the input file
#input_file = 'Izmir_multi.TIF'
#input_file = 'LT51800331984164XXX04_city_adj.TIF'
segmentation_file = 'watershed_multi.TIF'    #name of the raster file containing segmentation results
...
segmentation_vector_file = 'watershed_multi.shp'
output_type = 'table'    #type of the produced output (table for vector, segment for raster); default is table
output_vector_file = 'watershed_multi_features.shp' #name of the output shapefile
...
#print 'Watershed'
#watershed_otb(path+input_file,path+segmentation_vector_file,'vector',0,0.2)
#print 'Shp2Rast'
#rows,cols,nbands,geotransform,projection = Read_Image_Parameters(path+input_file)
#Shp2Rast(path+segmentation_vector_file,path+segmentation_file,rows,cols,'DN')

if os.path.exists(path + output_vector_file):
    os.remove(path+output_vector_file)
    
rows,cols,nbands,seg_list,geo_transform,projection = Read_Image(path+segmentation_file,np.uint16)
...
'''
print band_sum.shape
print rows,cols
print seg_list[0].shape
seg_data = seg_list[0].reshape(rows,cols)
...
if (output_type == 'table'):
    ...
    while infeature:
        ...
        for b in range(1,nbands+1):
            ...
            mean,std,maxbr,minbr,mode = spectral_features(dn,input_list[b-1],seg_list[0])
            ...
            contrast,energy,homogeneity,correlation,dissimilarity,asm = textural_features(dn,input_list_tf[b-1],seg_list[0])
            outfeature.SetField('Contr' + str(b),contrast)
            #outfeature.SetField('Energy' + str(b),energy)
            ...
            #outfeature.SetField('Asm' + str(b),asm)
        
        if (nbands>1):
            ndvi_mean,ndvi_std,wb = multispectral_features(dn,band_sum,ndvi,seg_list[0],nbands)

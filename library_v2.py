...
def classification(path,folder,n_class,location,mapset):
    ...
    grass.run_command("r.in.gdal", input = path+folder+'/'+b1_file[0], output = folder+'_B1_city_adj', flags = 'k')
    grass.run_command("r.in.gdal", input = path+folder+'/'+b2_file[0], output = folder+'_B2_city_adj', flags = 'k')
    grass.run_command("r.in.gdal", input = path+folder+'/'+b3_file[0], output = folder+'_B3_city_adj', flags = 'k')
    grass.run_command("r.in.gdal", input = path+folder+'/'+b4_file[0], output = folder+'_B4_city_adj', flags = 'k')
    grass.run_command("r.in.gdal", input = path+folder+'/'+b5_file[0], output = folder+'_B5_city_adj', flags = 'k')
    grass.run_command("r.in.gdal", input = path+folder+'/'+b6_file[0], output = folder+'_B6_city_adj', flags = 'k')
    grass.run_command("r.in.gdal", input = path+folder+'/'+b7_file[0], output = folder+'_B7_city_adj', flags = 'k')
    ...
    grass.run_command("r.out.gdal", input=output_name, output=path+folder+'/'+output_name+'.TIF', format='GTiff') #create the output image
...
def urban_development_landsat(path,folder):
    ...
    urban_index = ((mat_data2-mat_data5) / (mat_data2+mat_data5+0.0001)) #compute the urban_index
...
def pca(path,folder):
    ...
    return immean,mode,sec_order,third_order
